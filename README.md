...
Build the image:
```bash
docker build -t swe-backend:dev -f Dockerfile .
```

Run the container:
```bash
docker run --rm -p 5000:5000 --env-file .env swe-backend:dev
```

If the DB is started using the docker-compose.yml in the project, you need to attach the swe-backend container to the db containers network. In this case you do not need to set the `MONGO_HOST` and `MONGO_PORT` variables
```bash
docker run --rm -p 5000:5000 --network swe-backend_mongo --env-file .env swe-frontend:dev
```
